  ...
  def ToString(self):
    return '%s\n\n%s\n\n%s\n\n%s\n\n%s' % (
        ...
        '\n'.join(
            [
                key + ':' + value
                for key, value in self.anonymization_info.items()]))
...
def ReadDownloadedUrl(url, corpus):
  ...
  try:
    with open('%s/downloads/%s.html' % (corpus, Hashhex(url)), 'rb') as f:
      raw_data = f.read()
...
def DownloadUrl(url, corpus, max_attempts=5, timeout=5):
    ...
    file_path = os.path.join(corpus, 'downloads', f'{Hashhex(url)}.html')
    ...
    while attempts < max_attempts:
        try:
            ...
            if req.status_code == requests.codes.ok:
                content = req.content  # Use content to get bytes
                os.makedirs(os.path.dirname(file_path), exist_ok=True)
                with open(file_path, 'wb') as f:
                    f.write(content)
                return content.decode(req.encoding)
...
def ParseHtml(story, corpus):
  ...
  # Chuyển đổi chuỗi story.html sang byte
  encoded_html = story.html.encode()

  parser = html.HTMLParser(encoding=chardet.detect(encoded_html)['encoding'])
  tree = html.document_fromstring(encoded_html, parser=parser)
  ...
  def ExtractText(selector):
    ...
    paragraphs = [e.text_content() for e in elements]  # Giữ nguyên chuỗi

    # Remove editorial notes, etc.
    if corpus == 'cnn' and len(paragraphs) >= 2 and '(CNN)' in paragraphs[1]:
      paragraphs.pop(0)

    paragraphs = [s.strip() for s in paragraphs]  # Sử dụng strip cho chuỗi
    paragraphs = [s for s in paragraphs if s and not s.isspace()]  # Sử dụng isspace cho chuỗi
